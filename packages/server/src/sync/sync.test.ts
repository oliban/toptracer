...
describe('syncAll — incremental caching', () => {
  it('fetches everything the first time, then skips cached sessions', async () => {
    ...
    const first = await syncAll(client, { gameModes: ['WhatsInMyBag'] });
    ...
    client.byIdsFetched = [];
    const second = await syncAll(client, { gameModes: ['WhatsInMyBag'] });
  ...
  it('fetches a newly added session only', async () => {
    ...
    const client = new MockClient(server);
    await syncAll(client, { gameModes: ['WhatsInMyBag'] });
    ...
    client.byIdsFetched = [];
    const res = await syncAll(client, { gameModes: ['WhatsInMyBag'] });
    ...
    expect(getSessions().map((s) => s.id).sort()).toEqual(['A', 'C']);
    expect(getShots()).toHaveLength(3);
  });
  ...
  it('re-fetches a session whose traced-shot count grew', async () => {
    ...
    const client = new MockClient(server);
    await syncAll(client, { gameModes: ['WhatsInMyBag'] });
    expect(getShots()).toHaveLength(1);
    ...
    expect(getShots()).toHaveLength(2);
  });
});
