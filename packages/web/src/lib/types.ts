// Domain types — copied from packages/server/src/types.ts (kept in sync manually).
...
export interface FilterOptions {
  ...
  cleanHit: {
    ...
    // manual mode
    maxOffline: number; // SPREAD: |offline| beyond this many meters = bad hit
    shortfallPct: number; // LENGTH: % short of the club's typical distance = bad hit
  };
}
...
export interface FilteredShot extends Shot {
  ...
  excluded: boolean;
  excludeReason: 'hidden' | 'distanceOutlier' | 'lateralOutlier' | 'noData' | null;
}
